...
ros::Subscriber vicon_sub;		// vicon data topic subscriber
...
geometry_msgs::TransformStamped vicon_statedata;  	// Using this class variable for storing state data for Vicon simulator
...
void ViconStateCallback(const geometry_msgs::TransformStamped::ConstPtr & vicon_state_message); //callback function for vicon data 
...
while(ros::ok()){
   //actual vals for vicon
    double x = vicon_statedata.transform.translation.x, y = vicon_statedata.transform.translation.y, z = vicon_statedata.transform.translation.z;
